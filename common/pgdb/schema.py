...
DDL_TABLES: List[str] = [
    ...
    #
    # F-012（多站点）：本次**有意**加了第 56 列 ``marketplace`` 并把唯一键从
    # ``asin`` 换成 ``(asin, marketplace)``。这是一次声明过的契约变更，
    # 按 tests/golden/run.py 的规矩办：提交信息写明改了什么、为什么 -> 重录基线
    # -> 基线 blob 的 diff 就是评审物。
    #
    # 为什么非改不可：``asin`` 上那个 UNIQUE 的语义是「全库每个 ASIN 只有一行
    # 快照」（tests/test_multi_zip_same_asin.py 把它写成了成文事实）。同一个
    # ASIN 在美国站和加拿大站是**两件不同的商品数据** —— 价格、币种、库存、
    # 配送、卖家全不同。留着单行唯一键，两个站点的数据就会互相覆盖，
    # 后采的盖掉先采的，而且不报错、值还长得像真的。这正是 P4-8 那段
    # 「site 的值域本次有意不动」里预留下来、留给 Phase 5 的那笔账。
    #
    # ⚠ 为什么不复用既有的 ``site`` 列当这个维度：``site`` 实际写的是 ``"US"``
    #   （worker/parser.py 的 _default_result），与 ``scrape_events.marketplace``
    #   的 ``'amazon.com'`` 不是一个值域，而且它是**已交付的导出列**
    #   （common/config.py 的 HEADER_MAP "站点"）。拿它当键就得先回填全部历史行，
    #   而 asin_data 每 ASIN 只有一行、没有版本，回填之后无法回滚
    #   —— parser.py 的 P4-8 逐条论证过，那笔账这次仍然不动。
    #   新列与 scrape_events.marketplace 同名同值域，两者可以直接 join。
    f"""
    CREATE TABLE IF NOT EXISTS asin_data (
        ...
        -- F-012：采集来源站点。**新的最后一列**（上面 subtitle 那段
        -- "必须是最后一列"的警告现在指向这一行 —— 规则是"新列只能追加到末尾"，
        -- 标记永远跟着当前的末列走）。
        --
        -- NOT NULL + DEFAULT 'amazon.com' 是有意的：它是唯一键的一部分，
        -- 而 PG 的 UNIQUE 约束下 NULL 互不相等 —— 允许 NULL 就等于允许
        -- 同一个 ASIN 插进任意多行「站点未知」的快照，把这次要修的覆盖问题
        -- 换成一个更难查的重复问题。老库 ALTER 上来的行全部落到默认值，
        -- 与它们的事实一致（改造前只采美国站）。
        marketplace                 text COLLATE "C" NOT NULL DEFAULT 'amazon.com',
    ...
    # ---------------- tasks（18 列）----------------
    # F-012：``marketplace`` 是第 18 列，落在**末尾**。新列只能落末尾是本表
    # 的既定规则（verify_schema 逐位比对 ordinal_position），而且老库走的是
    # ALTER 路径，ALTER 出来的列必然在末尾 —— 写在中间会让新库与老库的列序
    # 不同，那比少一列更难查。
    f"""
    CREATE TABLE IF NOT EXISTS tasks (
        ...
        marketplace      text COLLATE "C" NOT NULL DEFAULT 'amazon.com',
        -- F-012：唯一键**故意仍是 (batch_id, asin)**，没有把 marketplace 加进去。
        -- 理由是它与 zip_code 的既定约定一致：一个批次一个采集语境
        -- （tests/test_multi_zip_same_asin.py 把这条写成了成文事实 ——
        --  「同 ASIN 多邮编这个诉求在单个批次内根本表达不了」，
        --  正确用法是一个邮编推一个批次）。站点同理：一个批次一个站点。
        -- 把 marketplace 加进唯一键会允许「一个批次里混着两个站点」，
        -- 而批次是进度、回调、截图目录、导出的共同单位 —— 混站点的批次
        -- 在这四处都会变成一个说不清的东西。
...
DDL_ALTERS: List[str] = [
    ...
    # ---------------- F-012：多站点 ----------------
    # 两条加列是常规的 ADD COLUMN IF NOT EXISTS。NOT NULL + DEFAULT 在 PG 11+
    # 不重写表（默认值存在 catalog 里），所以对千万行的 asin_data 也是秒级。
    'ALTER TABLE tasks ADD COLUMN IF NOT EXISTS marketplace '
    '  text COLLATE "C" NOT NULL DEFAULT \'amazon.com\'',
    'ALTER TABLE asin_data ADD COLUMN IF NOT EXISTS marketplace '
    '  text COLLATE "C" NOT NULL DEFAULT \'amazon.com\'',

    # 唯一键从 (asin) 换成 (asin, marketplace)。
    #
    # ⚠ 这条**不能**写成两句裸 ALTER。DDL_ALTERS 里的语句失败会照常抛
    #   （上面 init_tables 的注释写明了：这里只放"真幂等、失败会照常抛"的
    #   语句），而裸的 DROP CONSTRAINT 在已经迁过的库上就是一次失败。
    #   所以用 DO 块，按 pg_constraint 的实际状态决定做不做。
    #
    # ⚠ 顺序是**先建新的、再删旧的**，不是反过来：中间任何一刻都至少有一个
    #   唯一约束在守着 asin，不会出现一个"谁都能插重复行"的窗口。
    #   ON CONFLICT 的仲裁者在切换期间两个都在，(asin, marketplace) 能命中新的。
    #
    # 新约束显式命名 asin_data_asin_marketplace_key，与 PG 自动生成的名字
    # 一致；写出来是为了让下面的 IF NOT EXISTS 判定有个确定的名字可查，
    # 不依赖"PG 会怎么起名"这个实现细节。
    """
    DO $$
    BEGIN
        IF NOT EXISTS (
            ...
            WHERE conrelid = 'asin_data'::regclass
              AND conname  = 'asin_data_asin_marketplace_key'
        ) THEN
            ALTER TABLE asin_data
                ADD CONSTRAINT asin_data_asin_marketplace_key
                UNIQUE (asin, marketplace);
        END IF;
        ...
        IF EXISTS (
            SELECT 1 FROM pg_constraint
            WHERE conrelid = 'asin_data'::regclass
              AND conname  = 'asin_data_asin_key'
        ) THEN
            ALTER TABLE asin_data DROP CONSTRAINT asin_data_asin_key;
        END IF;
    END $$;
    """,
]
...
EXPECTED_COLUMNS: Dict[str, List[str]] = {
    ...
    "asin_data": [
        ...
        # 2026-08 / 2026-09 / F-012 追加，见 DDL 里的说明：新列只能落在末尾。
        "subtitle", "offer_condition", "marketplace",
    ],
    "asin_changes": [
